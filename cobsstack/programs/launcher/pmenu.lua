...
local pmsg = fs.list("/rom/programs")
...
for i=1,#pmsg do
    if fs.isDir("/rom/programs/"..pmsg[i]) then
        --Skip as we do not want to list dirs.
    else
...
function drawMenu()
    ...
    term.clearLine()
    term.setCursorPos(1,1)
    print("CobsStack -- Programs -- "..textutils.formatTime(os.time()).." Opt:"..SelectedOption.."          ")
    term.setTextColor(colors.white)
    term.setBackgroundColor(colors.black)
    term.setCursorPos(1,1)
...
while ctm == 1 do
    ...
    if event == "key" and param == 28 then
       ...
       term.clear()
        ...
        term.clearLine()
        term.setCursorPos(1,1)
        print("CobsStack -- "..pms[SelectedOption].." -- "..textutils.formatTime(os.time()).." Opt:"..SelectedOption.."              ")
        term.setTextColor(colors.white)
        term.setBackgroundColor(colors.black)
        print("")
        ...
        if pms[SelectedOption] == "Return to Menu" then
            ...
            term.setBackgroundColor(colors.black)
            print("")
            shell.run(pms[SelectedOption],pmsargs)
            term.setBackgroundColor(colors.white)
            term.setTextColor(colors.black)
